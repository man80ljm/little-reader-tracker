...
// ── 象形演化数据：甲骨文 → 金文 → 现代汉字 ────────────────────────────────
// SVG viewBox="0 0 40 40"，模拟古文字的象形图画风格
interface EvolutionStage {
  label: string;
  svg: string; // inner SVG content (paths/shapes)
  desc: string; // 对小朋友的简短说明
}
interface CharEvolution {
  oracle: EvolutionStage; // 甲骨文
  bronze: EvolutionStage; // 金文
}

const CHAR_EVOLUTION: Record<string, CharEvolution> = {
  山: {
    oracle: {
      label: "甲骨文",
      desc: "刻在龟壳上的三座山形",
  ...
  木: {
    oracle: {
      ...
      svg: `<line x1="20" y1="4" x2="20" y2="36" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/>
            ...
            <path d="M9,22 L20,18 L31,22" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round" opacity="0.5"/>`,
    },
    bronze: {
      label: "金文",
      desc: "树形更方正，根向两侧",
  ...
  火: {
    oracle: {
      ...
      svg: `<path d="M20,36 Q14,28 16,20 Q20,14 20,8 Q20,14 24,20 Q26,28 20,36" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
            <path d="M10,30 Q13,26 16,28" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round"/>
            <path d="M30,30 Q27,26 24,28" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round"/>
  ...
  人: {
    oracle: {
      label: "甲骨文",
      desc: "侧身站立、弯腰的人形",
      svg: `<path d="M22,6 Q24,10 22,14" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round"/>
            <path d="M22,14 Q18,20 14,28" stroke="currentColor" stroke-width="2.5" fill="none" stroke-linecap="round"/>
            <path d="M22,14 Q26,22 30,34" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round"/>
            <circle cx="22" cy="5" r="3" fill="currentColor" opacity="0.7"/>`,
    },
    bronze: {
      label: "金文",
      desc: "两腿撑地，变成撇和捺",
...
// ── 象形演化条组件 ────────────────────────────────────────────────────────
function CharEvolutionStrip({ char }: { char: string }) {
  const data = CHAR_EVOLUTION[char];
  if (!data) return null;

  const stages = [
    { ...data.oracle, char: undefined, isModern: false },
    { ...data.bronze, char: undefined, isModern: false },
    { label: "现代汉字", desc: "今天我们写的字", svg: undefined, isModern: true },
  ];
              ...
              >
                {stage.isModern ? (
                  ...
                  <svg
                    ...
                    height="32"
                    style={{
                      color: i === 0 ? "hsl(35 65% 45%)" : "hsl(200 55% 50%)",
                    }}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-2xl bg-card border-2 border-border p-4">
        ...
        {/* 象形演化条 */}
        <CharEvolutionStrip char={task.target} />
      </div>
